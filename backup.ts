...
startServer()} }
  app.get('/api/server-folders/browse', verifyToken, async (req, res) => {
    try {
      ...
      const targetPath = path.join(settings.rootPath, dir)} }
  ...
  app.post('/api/server-folders/create', verifyToken, async (req, res) => {
    try {
      ...
      if (targetDir?.includes('..') || newFolderName?.includes('..')) {
        return res.status(403).json({ error: "Geçersiz yol" )} }
      

      const sanitizedNewFolder = sanitizePath(newFolderName)} }
      const fullPath = path.join(settings.rootPath, targetDir || '', sanitizedNewFolder)} }
  ...
  app.post('/api/server-folders/save-file', verifyToken, async (req, res) => {
    try {
      ...
      if (targetDir?.includes('..') || filename?.includes('..')) {
        return res.status(403).json({ error: "Geçersiz yol" )} }
      ...
      // target directory
      const fullTargetDir = path.join(settings.rootPath, targetDir || '')} }

      if (!fs.existsSync(fullTargetDir)) {
        await fs.promises.mkdir(fullTargetDir, { recursive: true )} }
      

      const targetPath = path.join(fullTargetDir, safeFilename)} }
      ...
      while (fs.existsSync(finalPath)) {
        finalFilename = `${baseName_${counter}${ext}`} }
        finalPath = path.join(fullTargetDir, finalFilename)} }
        counter++} }
